...
def parse_input(type):
    while True:
        number = input("Please specify the " + type + " in pixels of each sample: ")
        try:
            val = int(number)
            if val < 0:
...
if __name__ == "__main__":
    ...
    input("A menu will now appear, from which an image should be selected.\nPress Enter when ready.")
    root = tk.Tk()
    root.withdraw()
    ...
    # At this point, the sample calculator calculates the set of coordinates for generating each sample.
    image_positions = sample_calculator.run(width, height, sample_width, sample_height, SAMPLE_SIZE)
    print(image_positions)
    images = []
    for i in image_positions:
        cropped_image = crop_image(image, sample_width, sample_height, i[0], i[1])
        images.append(cropped_image)
        
        # Each cropped image is displayed using the system's default image viewer.
        cropped_image.show()
